...
## 6.setTimeout、Promise、Async/Await 的区别 -- 2021/03/01
...
## 11. JSONP可以跨越吗？JSONP跨域的原理
```
JSONP是通过动态<script>元素使用的，可以为src属性指定一个跨域URL，<script>有能力从其他域加载资源。
```

## 12. 两次上传同样的文件怎么触发onchange
```
...
## 8.  解释一下redux里面的reducers

```
数据的状态是通过reducer函数来改变的。
```

## 9. 解释一下redux里面的actions
...
## 10. react-hook的useMemo和useCallBack有什么区别
...
## 2. Chrome与Node的事件循环(Event Loop)有何区别

Chrome和node: https://juejin.cn/post/6844903761949753352
